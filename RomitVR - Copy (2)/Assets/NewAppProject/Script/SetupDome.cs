...
public class SetupDome : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        DisableCenterpanalButtons();
    }

    public void DisableCenterpanalButtons() {
        Debug.Log(CenterBottomPanal.transform.childCount);
        for (int i = 0; i < CenterBottomPanal.transform.childCount; i++) {
            Debug.Log(CenterBottomPanal.transform.transform.GetChild(i).gameObject.name);
            CenterBottomPanal.transform.GetChild(i).GetChild(0).GetComponent<Button>().interactable = false;
        }

    }
    public void EnableCenterpanalButton()
    {

        for (int i = 0; i < CenterBottomPanal.transform.childCount; i++)
        {

            CenterBottomPanal.transform.GetChild(i).GetChild(0).GetComponent<Button>().interactable = true;
        }
            ...
            for (int i = 0; i < PreviewLoader.SenceTexturePath.Count; i++)
            {
                ...
                for (int j = 0; j < HotspotContainer.transform.GetChild(i).childCount; j++)
                {
                    ...
                    if (HotspotContainer.transform.GetChild(i).GetChild(j).GetComponent<helpActionHotspot>())
                    {
                        ...
                        PreviewLoader.ScenesData[i].HotspotData[j].Always = HotspotContainer.transform.GetChild(i).GetChild(j).GetComponent<helpActionHotspot>().Always;
                        PreviewLoader.ScenesData[i].HotspotData[j].isRequire = HotspotContainer.transform.GetChild(i).GetChild(j).GetComponent<helpActionHotspot>().isRequire;
