		...
		if (SetupDome.SelectedHotspot == gameObject) {
			...
			for (int i = 0; i < change.options.Count; i++) { 
				...
				if (change.options [i].text == change.captionText.text) {
					...
					locationDome.transform.GetComponent<MeshRenderer> ().material.mainTexture = NewHotspotContainer.transform.GetChild (i).GetComponent<SceneProperties> ().SceneTexture;
