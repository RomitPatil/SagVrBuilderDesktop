		...
		for (int i = 0; i < HotspotContainer.transform.childCount; i++) {
			...
			if (HotspotContainer.transform.GetChild (i).name == DomeSetup.Scene_Name_Input.text) {
				...
				for (int j = 0; j < HotspotContainer.transform.childCount; j++) {
					hotspotObj.GetComponent<NewActionHotspot> ().SceneTexture.Add (HotspotContainer.transform.GetChild (j).GetComponent<SceneProperties> ().SceneTexture);
					hotspotObj.GetComponent<NewActionHotspot> ().SceneTexturePath.Add (HotspotContainer.transform.GetChild (j).GetComponent<SceneProperties> ().SceneTexturePath);
				}   
