...
namespace TriLib
{
    namespace Samples
    {
        ...
        public class OpenFileImages : MonoBehaviour {
            ...
            public void OnClick()
            {
                ...
                if (paths.Length > 0)
                {
                    ...
                    for (int i = 0; i < paths.Length; i++)
                    {
                        ...
                        StartCoroutine(OutputRoutine(new System.Uri(paths[i]).AbsoluteUri));
            ...
            {
                Debug.Log(path + ";;;;;;;;");
                StartCoroutine(OutputRoutineTwo(path, name, UnityImage));
            }

            public IEnumerator OutputRoutineTwo(string url, string name, bool UnityImage)
            {
                ...
                if (UnityImage == false)
                {
                    ...
                    if (ExtentionName == "png" || ExtentionName == "jpg" || ExtentionName == "PNG" || ExtentionName == "jpeg")
                    {
                ...
                if (UnityImage == false)
                {
                    if (ExtentionName == "mp4" || ExtentionName == "MP4")
                    {
                        ...
                        if (loadMovie.error == null)
                        {
                            ...
                            Debug.Log("file name time n " + name);
                            i++;
                            footag.transform.GetComponent<RawImage>().texture = loadMovie.texture;
                            FootageDragAndDrop.ModelsFiles.Add(footag);
                            footag.transform.GetComponent<SelectFiles>().inPhotoViewer = false;
                ...
                if (UnityImage)
                {
                    ...
                    if (ExtentionName == "png" || ExtentionName == "jpg" || ExtentionName == "PNG" || ExtentionName == "jpeg" || ExtentionName == "fbx" || ExtentionName == "obj" || ExtentionName == "FBX")
                    {
                     
                        if (ExtentionName == "png" || ExtentionName == "jpg" || ExtentionName == "PNG" || ExtentionName == "jpeg")
                        {
                            CacheModelImage.Add(url);
                            loadUM = new WWW(CacheModelImage[CacheModelImage.Count - 1]);
                            Debug.Log("file name time 0 " + fileName);
                            yield return loadUM;
                            Texture2D T = loadUM.texture;
                            ...
                            if (ExtentionName == "fbx" || ExtentionName == "obj" || ExtentionName == "FBX")
                            {
                                ...
                                CacheModelFiles.Add(url);

                                var loadUnity = new WWW(CacheModelFiles[CacheModelFiles.Count - 1]);
                                Debug.Log("file name time 0 " + fileName);
                                yield return loadUnity;
                                Debug.Log("file name time 1 " + fileName);

                                if (loadUnity.error == null)
                                {
                                    int i = 0;
                                    FootageDragAndDrop.FileName.Add(name);
                                    GameObject footag = GameObject.Instantiate(FootageObjectPrfb);
                                    ...
                                    footag.transform.localPosition = new Vector3(0, 0, 0);
                                    footag.transform.localScale = new Vector3(1, 1, 1);
                                footag.gameObject.name = name;
                                    Debug.Log("Sucess Image Asset");
                                    footag.transform.GetChild(0).GetComponent<InputField>().text = name;
                                    Debug.Log("file name time n " + name);
                                    i++;
                                    footag.transform.GetComponent<RawImage>().texture = loadUM.texture;
                                    FootageDragAndDrop.ModelsFiles.Add(footag);
                                    footag.transform.GetComponent<SelectFiles>().inPhotoViewer = false;
                                    footag.transform.GetComponent<SelectFiles>().UnityObject = true;
                                footag.transform.GetComponent<SelectFiles>().UnityAssetPath = url;
                                FootageDragAndDrop.UnityAssetPath.Add(url);
                                Debug.Log( CacheModelFiles[CacheModelFiles.Count - 1] );
                                string remove = CacheModelFiles[CacheModelFiles.Count - 1];
                                    string FIlepath = remove.Replace(Path.GetFileName(remove), "");
                                    Debug.Log(FIlepath + "^^^^");
                                ...
                                {
                                    Debug.Log("Error" + loadUnity.error);
                                }
...

                {
                    ...
                    if (ExtentionName == "mp4" || ExtentionName == "MP4")
                    {
                        ...
                        File.WriteAllBytes(currentFolderPath + "/" + "VideoFile" + "/" + fileName, moveFile);

                        string path = new System.Uri(currentFolderPath + "/" + "VideoFile" + "/" + fileName).AbsoluteUri;
                        ...
                        if (load.error == null)
                        {
                            ...
                            FootageDragAndDrop.FileName.Add(fileName);
                            GameObject footag = GameObject.Instantiate(FootageObjectPrfb);
                            footag.transform.parent = FootageContainer.transform;
                            footag.transform.localPosition = new Vector3(0, 0, 0);
                            footag.transform.localScale = new Vector3(1, 1, 1);
                            ...
                            footag.transform.GetComponent<SelectFiles>().inPhotoViewer = false;
                            footag.transform.GetComponent<SelectFiles>().Video = true;
                            footag.transform.GetComponent<SelectFiles>().MediaFile = CacheMovieFiles[CacheMovieFiles.Count - 1];
                        ...
                        ExtentionName == "JPEG")
                    {
                        ...
                        File.WriteAllBytes((currentFolderPath + "/" + "ImageFile" + "/" + fileName), ImageFile);

                        string path = new System.Uri(currentFolderPath + "/" + "ImageFile" + "/" + fileName).AbsoluteUri;
                        ...
                        ExtentionName != "JPEG" || ExtentionName != "mp4" || ExtentionName != "MP4")
                    {
                        ...
                        string path = currentFolderPath + "/" + "UnityFile" + "/" + fileName;
